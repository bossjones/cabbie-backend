...
class AbstractAndroidApplicationVersion(ActiveMixin):
...
class AndroidDriver(AbstractAndroidApplicationVersion):
    
    class Meta(AbstractAndroidApplicationVersion.Meta):
        verbose_name = u'안드로이드 기사앱 버전관리'
        verbose_name_plural = u'안드로이드 기사앱 버전관리'

class AndroidPassenger(AbstractAndroidApplicationVersion):
