...
def run():
    start_time = datetime.datetime.strptime('2015-11-15 18:00', "%Y-%m-%d %H:%M")
    end_time = datetime.datetime.strptime('2015-11-23', "%Y-%m-%d")
    ...
    for ride in rides:
        if ride.driver:
            first_ride = ride.driver.rides.order_by('id')[0] # not crash
